...
SRC_URI="
	...
	https://www.unicode.org/Public/13.0.0/ucd/UnicodeData.txt -> UnicodeData-13.0.0.txt
"
...
PATCHES=(
	"${FILESDIR}"/c12e8515.patch
	"${FILESDIR}"/julia-1.9.4-string-replace.patch
	"${FILESDIR}"/julia-libcholmod-cuda.patch
	"${FILESDIR}"/julia-hardcoded-libs.patch
	"${FILESDIR}"/julia-1.10.1-no-doc-install.patch
	)
...
src_unpack() {
	...
	cp "${DISTDIR}/UnicodeData-13.0.0.txt" "${S}/deps/srccache/" || die

	# Extract tarballs for patching.
	# cd "${S}/deps/srccache/" || die
	# tar xf llvm-julia-${MY_LLVM_V}-10.tar.gz || die
}
