...
#linux-UFRII-drv-v560-m17n-10.tar.gz
CANON_REL="11"
#DL_ID="8/0100007658/25"
#DL_ID="8/0100765801/30"
#DL_ID="8/0100007658/31"
DL_ID='8/0100007658/33'
MY_PV="${PV//\./}"
...
src_install() {
	...
	#dosym libColorGearC.so.2.0.0 /usr/$(get_libdir)/libColorGearC.so
	#dosym libColorGearC.so.2.0.0 /usr/$(get_libdir)/libColorGearC.so.2
	...
	insinto /usr/share/cnsetuputil2
	doins "${S}"/cnrdrvcups-utility-${PV}/data/cnsetuputil.ui
	...
	mkdir -pv "${ED}/usr/$(get_libdir)/cups/backend"
	mv "${ED}/usr/lib/cups/backend/cnusb" "${ED}/usr/$(get_libdir)/cups/backend/" || die
	dosym ../../../$(get_libdir)/cups/backend/cnusb /usr/libexec/cups/backend/cnusb

	mkdir -pv "${ED}/usr/$(get_libdir)/cups/filter"
	mv "${ED}/usr/lib/cups/filter/pdftocpca" "${ED}/usr/$(get_libdir)/cups/filter/" || die
	dosym ../../../$(get_libdir)/cups/filter/pdftocpca /usr/libexec/cups/filter/pdftocpca

	mv "${ED}/usr/lib/cups/filter/rastertoufr2" "${ED}/usr/$(get_libdir)/cups/filter/" || die
